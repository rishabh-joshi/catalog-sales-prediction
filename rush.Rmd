...
linear2 <- lm(targdol ~ I(log(slstyr + K)) + I(log(slslyr + K)) +
                  ...
                  recency:active + 
                  I(log(ordlyr + K)):I(log(ord2ago + K)):I(log(ord3ago + K)) +
                I(log(slslyr + K)):I(log(sls2ago + K)) + 
                I(log(sls2ago + K)):I(log(sls3ago + K)) +
...
# uncomment this line if the response of the model is log
# pred_test <- exp(pred_test)-K
targdol_test <- data.frame(id = test_linear$id, targdol = pred_test)
```

This model esults in a lower payoff and higher MSPE.

### Lasso regression (not completed yet)


```{r}
linear_model <- linear2

train_lasso <- model.matrix(formula(linear_model$call), data = train_linear)
test_lasso <- model.matrix(formula(linear_model$call), data = test_linear)

lassocv <- cv.glmnet(train_lasso, 
                     train_linear$targdol, alpha=1, nfold=3,
...
print(small.lambda.betas[small.lambda.betas>0])
...
linear_model <- bwd # change this to any linear model you tried
...
```{r}
th_max_payoff <- sum(sort(prob_targdol$actual_targdol, 
